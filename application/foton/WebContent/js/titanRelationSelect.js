	function TitanRelationSelect() {
		this.urls = [titanServerPath + '/core::/commonModule/commonAction/GetDict.action',titanServerPath + '/core::/commonModule/commonAction/GetDict.action'];
	...
	$.extend(TitanRelationSelect.prototype, {	
		...
		execute:function() {			
			...
			var level_1_change = function(change) { 
				...
				Titan.getLevel2 = function(msg) {
					...
					Titan.getLevel3_2 = function(msg) {
						if(msg.id){
							var optionsStr = '';
							$.each(msg[root], function(index,data){	
								if(msg.key == data[key]){
					...
					if ($('#' + msg.id).val() && msg.level_3_id){
						Titan.executeAction(unescape(msg.level_3_url) + "execute=getLevel3_2&level=2&value=" + $('#' + msg.id).val() + "&id=" + msg.level_3_id + "&key=" + msg.level_3_key);
					}
				}
				if (level_1.val() != ""){
					if(level_3){
						Titan.executeAction(url1 + "execute=getLevel2&level=1&value=" + level_1.val() + "&id=" + level_2.attr('id') + "&key=" + defaultValue[1] + '&level_3_id=' + level_3.attr('id') + '&level_3_url=' + escape(url2) + '&level_3_key=' + defaultValue[2]);	
					} else {
						Titan.executeAction(url1 + "execute=getLevel2&level=1&value=" + level_1.val() + "&id=" + level_2.attr('id') + "&key=" + defaultValue[1]);
					}
				}
			};
			...
			var level_2_change = function(change){
				...
				if (level_2.val()){
					if(level_3){
						Titan.executeAction(url2 + "execute=getLevel3&level=2&value=" + level_2.val() + "&id=" + level_3.attr('id') + "&key=" + defaultValue[2]);
					} else {
						Titan.executeAction(url2 + "execute=getLevel3");
					}
				}
			}
 	...
 	$.fn.titanRelationSelect = function(options){
		var rs = new TitanRelationSelect();
		for(e in options){
			rs[e] = options[e];	
		}
		rs.execute();
