...
<script type="text/javascript">
		$(function() { 
			$("#paginate").saturnTablePaginate({
				...
				action:'${pageContext.request.contextPath}/crm::/dictModule/action/QueryWaiterList.action',
				form:'crmform',
				percount:10
			});
		});
		...
		function modify(type,code,hidden_flag){
	 	...
	 	function addWorker(){
			window.open("<%=request.getContextPath()%>/crm/dictModule/web/addWaiter.jsp", "", "height=300, width=800, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no");
		}
</script> 
<title>服务顾问管理</title>
</head>
<body>
<div id="man_zone">
		<form id="crmform" class="queryFrom" method="post" action="${pageContext.request.contextPath}/crm::/dictModule/action/QueryWaiterList.action">
			<input type="hidden" name="startIndex" value="0"/>
			<input type="hidden" name="endIndex" value="9"/>
			<fieldset id="formFiled">
			  <legend>服务顾问管理</legend>
               <table cellspacing="2" cellpadding="0">
                    <tr>
                      <td class="colName">顾问编号:</td>
                      <td class="detailColVal">
                      	<input type="text" name="code" id="code" value="${code}"/>
                      </td>
                      <td class="colName">顾问姓名:</td>
                      <td class="detailColVal">
