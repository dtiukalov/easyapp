...
public class UpdateUserType2 extends DefaultComponent {
	...
	public void execute(IParameterContext parameterContext) throws Exception {
		...
		String servid = (String)parameterContext.getParameterValue(2);
		usertype=getServCount(vin,servid);
		parameterContext.setParameter(0,usertype);//返回用户类别
	}
	
	public String getServCount(String vin,String servid){
			String usertype="";
			int i=0,j=0;
	      Statement stmt=null; 
	      ResultSet rs=null; 
	     try {
	    	 ...
	    	 if(vin!=null && !"".equals(vin.trim())){
	    		...
	    		strSql="select count(*) from crm_service where carvin='"+vin+"' and serviceid<>"+servid; 
	    		stmt=conn.createStatement(); 
				 rs=stmt.executeQuery(strSql); 
				 if(rs.next()){
