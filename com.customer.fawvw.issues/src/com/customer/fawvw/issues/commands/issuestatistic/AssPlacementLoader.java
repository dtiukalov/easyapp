...
public class AssPlacementLoader {
	...
	private static Map<String, Object> assPlacement = new HashMap<String, Object>();
	...
	public static  Map<String,Object> load(ArrayList<HashMap<String, Object>> values){
		...
		for(int k=0; k<values.size(); k++) {
			...
			if ("前端 Vor-derwagen".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
				front = sumLightStatue(front, (String)(values.get(k)).get("fv9RGStatus")); //$NON-NLS-1$
			}
			
			if ("后端 Heck".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
			...
			if ("内饰 Innenraum".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
				inner = sumLightStatue(inner, (String)(values.get(k)).get("fv9RGStatus")); //$NON-NLS-1$
			}
			
			if ("底盘 Boden".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
				chassis = sumLightStatue(chassis, (String)(values.get(k)).get("fv9RGStatus")); //$NON-NLS-1$
			}
			
			if ("驾驶模块 Cockpit".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
				driver = sumLightStatue(driver, (String)(values.get(k)).get("fv9RGStatus")); //$NON-NLS-1$
			}
			
			if ("电器 Elektronik".equals((String)(values.get(k)).get("fv9AssPlacement"))) { //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-1$
