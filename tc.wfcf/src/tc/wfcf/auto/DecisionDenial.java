...
public class DecisionDenial extends AutoProcessDecision implements
		AutoProcessor {
	...
	public DecisionDenial(String comments, long retryDelayOnFailure,
			boolean allowFurtherProcess) {
		super(retryDelayOnFailure);
		...
		if (task.isTypeOf("EPMPerformSignoffTask")) {
			...
			for (TCComponentSignoff signoff : task.getValidSignoffs()) {
				...
				if (session.getUser() == gm.getUser()) {
					signoff.updateDecision(TCCRDecision.REJECT_DECISION,
							comments, "");
					task.performAction(105 /* EPM_REJECT_ACTION */, comments);
					processCount++;
				}
			}
			return processCount > 0;
		} else if (task.isTypeOf("EPMDoTask")) {
			FailureOperation.triggerFailure(task, comments, null);
			return true;
		} else {
			return false;
		}
