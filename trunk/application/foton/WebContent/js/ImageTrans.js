 ...
 */
//容器对象
var ImageTrans = function(container, options, w, h){
	this._initialize( container, options, w, h);
	this._initMode();
	if ( this._support ) {
		this._initContainer();
		this._init();
...
ImageTrans.prototype = {
  //初始化程序
  _initialize: function(container, options, w, h) {
...
//this._img.height = h;
	//alert(this._img.src);
	//alert(this._img.width);
	//alert(this._img.height);
	//DrawImage3(w, h ,900,400);
	//alert(this._img.width);
	//alert(this._img.height);
//			alert(w);
//alert(h);
	...
	this._imgWidth = DrawImageW(w, h, 900, 400);//this._img.width;//emily
	this._imgHeight = DrawImageH(w, h, 900, 400);//this._img.height;	//emily

		//alert(this._imgWidth);
//alert(this._imgHeight);
	this._style = {};//备份样式
...
//	style.overflow = "hidden";  emily  
	$$CE.fireEvent( this, "initContainer" );
  },
  //加载图片
  load: function(src) {
	if ( this._support ) {
		var img = this._img, oThis = this; //img = new Image();
...
ImageTrans.modes = function(){
	...
	//初始化图片对象函数
	function initImg(img, container,w,h) {
		$$D.setStyle( img, {
			position: "absolute",
			border: 0, padding: 0, margin: 0, width: w + "px", height: h + "px",	//重置样式//emily
	...
	return {
		css3: {//css3设置
			...
			init: function() {initImg( this._img, this._container, this._imgWidth, this._imgHeight); },
			load: function(){
				var img = this._img;
				$$D.setStyle( img, {//居中
		...
		filter: {//滤镜设置
			...
			init: function() {
				...
				initImg( this._img, this._container, this._imgWidth, this._imgHeight);
				//设置滤镜
