	...
	*/
	...
	$.extend(SaturnTablePaginate.prototype, {
		printCode:function(j){
			 ...
			 if(type == 'action'){
			 	if(parseInt(name[1]) && !p && Saturn.pageName == name[0]){
					if(el){
						$('#' + el).html("");
						go(parseInt(name[1]), max, action, percount, form, type);
					}
			 	} else if(!p){
					if(el && $('#' + el).html() == ""){
			 ...
			 if(max > 1){
				 pCode += '<input class="clb-table-paginate-input" type="text" id="' + num + '" style="width:40px;text-align:center"';
			 ...
			 pCode += '<button style="color:#333; margin-left:10px;border:none; background:#faf7ee;margin-top:10px">每页' + percount + '条/共' + count + '条数据</button>'
