...
{	
	...
	function inventory_summary($export_excel=0)
	{
		...
		$sum_number = $model->getSumNumber();
		$sum_price = $model->getSumPirce();
		
		$suppliersheaders = $model->getSuppliersColumns();
		
		$supplier_info = array();
		$supplier = $model->getSuppliers(array());
		...
		$brands_data = "小计：";
		$sum_data = " 总数量：".$sum_number."   总金额：".$sum_price;
		...
		$data = array(
			...
			"sum_data" =>$sum_data,
			"suppliersheaders" =>$suppliersheaders
		);
