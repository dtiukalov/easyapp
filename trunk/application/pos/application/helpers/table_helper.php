...
function get_supplier_data_row($supplier,$controller)
{
	...
	$table_data_row.='<td width="7%">'.character_limiter($supplier->person_id,13).'</td>';
	$table_data_row.='<td width="10%">'.character_limiter($supplier->company_name,13).'</td>';
	$table_data_row.='<td width="10%">'.character_limiter($supplier->brand,13).'</td>';
	$table_data_row.='<td width="10%">'.character_limiter($supplier->first_name,13).'</td>';
	$table_data_row.='<td width="10%">'.character_limiter($supplier->last_name,13).'</td>';
...
function get_item_data_row($item,$controller)
{
	...
	$width = $controller->get_form_width();
	$aaa=$item->quantity;
	$bbb=$item->reorder_level;
	
	$i=1;
	if($aaa<=$bbb){
		$table_data_row='<tr style="background-color:#FF0033;">';
	}else{
		$table_data_row='<tr>';
	}
	...
	$table_data_row.='<td width="14%">'.$item->name.'</td>';
	$table_data_row.='<td width="11%">'.$item->category.'</td>';
	$table_data_row.='<td width="11%">'.to_currency($item->cost_price).'</td>';
	...
	$table_data_row.='<td width="38%">'.anchor($controller_name."/inventory/$item->item_id/width:$width", $CI->lang->line('common_inv'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_count')))./*'</td>';//inventory count	
	$table_data_row.='<td width="5%">'*/'&nbsp;&nbsp;&nbsp;&nbsp;</br>'.anchor($controller_name."/count_details/$item->item_id/width:$width", $CI->lang->line('common_det'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_details_count'))).'</td>';//inventory details	
