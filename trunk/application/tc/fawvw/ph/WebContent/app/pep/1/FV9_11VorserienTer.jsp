...
<html>
	...
	public static int getIndex(String date, int index, int year, int kw) {
		if (kw == DateUtils.getWeekOfYear(date + " 00:00")) {
			if ((year == Integer.parseInt(date.split("-")[0]))) {
				System.out.println("date = " + date + " index = " + index
						+ " year = " + year + " kw = " + kw);
				return index;
			}
			
			//特殊状况,如：2013-01-04是2012年的53周，此处取出date的year是2013，而实际的year应该是2012，无法匹配
			//解决方案：获取date所在周的第一天的年，进行匹配
			Date specific_date = DateUtils.transStringToDate(date + " 00:00"); //string转date
			Date specific_monday = DateUtils.getFirstDayOfWeek(specific_date); //指定日期的星期一
			Calendar cal = Calendar.getInstance(); 
			cal.setTime(specific_monday);
			int specific_year = cal.get(Calendar.YEAR); //星期一所在年
			if (year == specific_year) {
				System.out.println("specific_date ： date = " + date + " index = " + index
						+ " year = " + year + " kw = " + kw);
				return index;
			}
		}
	...
	public static boolean getYesOrNo(int l, List<Integer> indexlist) {
		...
		<%
			...
			if (Web.getObjectYesOrNo(form.get("fv9VFFVorBatches"))) {
				fv9VFFVorBatches = Integer.parseInt((String) form
						.get("fv9VFFVorBatches"));
			}
			int fv9VFFVorFzg = 0;
			if (Web.getObjectYesOrNo(form.get("fv9VFFVorFzg"))) {
				fv9VFFVorFzg = Integer.parseInt((String) form
						.get("fv9VFFVorFzg"));
			}
			int fv9VFFAbsMeter = 0;
			if (Web.getObjectYesOrNo(form.get("fv9VFFAbsMeter"))) {
				fv9VFFAbsMeter = Integer.parseInt((String) form
						.get("fv9VFFAbsMeter"));
			}
			...
			if (Web.getObjectYesOrNo(form.get("fv9PVSVorBatches"))) {
				fv9PVSVorBatches = Integer.parseInt((String) form
						.get("fv9PVSVorBatches"));
			}
			int fv9PVSVorFzg = 0;
			if (Web.getObjectYesOrNo(form.get("fv9PVSVorFzg"))) {
				fv9PVSVorFzg = Integer.parseInt((String) form
						.get("fv9PVSVorFzg"));
			}
			int fv9PVSAbsMeter = 0;
			if (Web.getObjectYesOrNo(form.get("fv9PVSAbsMeter"))) {
				fv9PVSAbsMeter = Integer.parseInt((String) form
						.get("fv9PVSAbsMeter"));
			}
			...
			if (Web.getObjectYesOrNo(form.get("fv90SVorBatches"))) {
				fv90SVorBatches = Integer.parseInt((String) form
						.get("fv90SVorBatches"));
			}
			int fv90SVorFzg = 0;
			if (Web.getObjectYesOrNo(form.get("fv90SVorFzg"))) {
				fv90SVorFzg = Integer
						.parseInt((String) form.get("fv90SVorFzg"));
			}
			int fv90SAbsMeter = 0;
			if (Web.getObjectYesOrNo(form.get("fv90SAbsMeter"))) {
				fv90SAbsMeter = Integer.parseInt((String) form
						.get("fv90SAbsMeter"));
			}
		...
		System.out.println("=====DATE_SOP = "+DATE_SOP);
		System.out.println("=====DATE_ME = "+DATE_ME);

			int SOP_KW = DateUtils.getWeekOfYear(DATE_SOP);
		System.out.println("=====SOP_KW = "+SOP_KW);
	...
	<body>
		<div id="container">
			...
			<%
				...
				if (!"1900-01-01 00:00:00".equals(fv9VFFTBTZP5)
						&& !"".equals(fv9VFFTBTZP5))
				...
				String flag = "sop";
				if (!DateUtils.compareTime(fv9PVSAbsEnd + " 00:00", DATE_SOP
						+ " 00:00"))
					flag = "me";
				
		System.out.println("=====flag = "+flag);

				if ("sop".equals(flag) && !DATE_SOP.startsWith("1900-01-01")
						&& !"".equals(DATE_SOP))
					endDate = DateUtils.getDateAddMonth(DATE_SOP, 1, "+");

				if ("me".equals(flag) && !DATE_ME.startsWith("1900-01-01")
						&& !"".equals(DATE_ME))
			...
			//	Date end = sdf.parse(endDate);
				...
				if (!"".equals(startDate) && !"".equals(endDate)) {
					...
					System.out.println("startDate = " + startDate + " - endDate = "
							+ endDate);
					System.out.println("startKW = " + startKW + " - endKW = "
							+ endKW);
					...
					for (int w = 0; w < weeks + 1; w++) { //多加一次循环，是为了计算年合并单元格数
						...
						String date = DateUtils.oneDateAddOrSubDays(beginTime,
								(w * 7), "+");
			...
			%>
				...
				<table style="border: 1px solid; width: <%=totalWidth%>px; height: 530px; margin: 20px 0px auto; 
					...
					<tr>
						...
						<td style="border: 1px solid; width: <%=totalWidth%>px; height:60px;" colspan="<%=weeks%>">
							<div style="float: left; width: 15%; height: 100%; margin: 0px; padding: 0px; overflow: hidden; text-align: left;">
					...
					%>
						<td style="border: 1px solid; width: <%=year.cols * tdWidth%>px; height:18px; overflow:hidden; 
							font-weight: bolder; background-color: #FFFFC0; font-size:12px;" 
							colspan="<%=year.cols%>">
					...
					%>
						<td style="border: 1px solid;width: <%=tdWidth%>px; height:15px; overflow:hidden; background-color: #FFFFC0;">
							 <%=timeinfo.kw%>
						</td>
					<%
						}
					...
					<%
						for (int i = sopIndex + 1; i > 0; i--) {
							String fontsize	= "10"; //默认数字大小
							if(i<-8 || i>99)  //数字超过3位
								fontsize = "8";
							
					%>
						<td style="border: 1px solid;width: <%=tdWidth%>px; height:15px; font-size: <%=fontsize %>px;
							<%if (i % 2 != 0)
					...
					%>
						...
						<%if (kw == 1) {
						out.print("border-left: 1px solid;");
					}%>	
						<%if ((l != timeList.size() - 1)
							&& (getYesOrNo(l, indexList))) {
				...
				<%
				//关键点的绝对高度
				//int baseTop = 110 + 20 + 60 + 18 + 15 + 15; 
				//title(100) + 表格与上面的空白（20） + 表格LOGO（60） + 年（18） + 周（15） + 与SOP倒数的周数（15）
				//最后的20px是我的机器与大众服务器的差异
						int baseTop = 110 + 20 + 60 + 18 + 15 + 5 + 20;
						int level1 = baseTop + 20;
						int level2 = level1;
				...
				<%
					if (VFFVORSTART != 0 && VFFVOREND != 0) {
							String vffdesc = fv9VFFVorBatches + "&nbsp;Batches("
									+ fv9VFFVorFzg + " Fzg.)";
				...
				<%
					if (PVSVORSTART != 0 && PVSVOREND != 0) {
							String pvsdesc = fv9PVSVorBatches + "&nbsp;Batches("
									+ fv9PVSVorFzg + " Fzg.)";
				...
				<%
					if (OSVORSTART != 0 && OSVOREND != 0) {
							String osdesc = fv90SVorBatches + "&nbsp;Batches("
									+ fv90SVorFzg + " Fzg.)";
							if (!"".equals(fv9PVSDesc)) {
								osdesc += "*";
							}
				...
				<%
						...
						if (!"".equals(fv9VFFDesc) || !"".equals(fv9PVSDesc)
								|| !"".equals(fv90SDesc)) {
							String desc = fv9VFFDesc + "<br>" + fv9PVSDesc + "<br>"
									+ fv90SDesc;
				%>
				...
				<%
						...
						System.out.println("VFFTBTZP5 = " + VFFTBTZP5 + " VFFTBTZP7 = "
								+ VFFTBTZP7 + " VFFVORSTART = " + VFFVORSTART
								+ " VFFVOREND = " + VFFVOREND + " VFFABSSTART = "
								+ VFFABSSTART + " VFFABSEND = " + VFFABSEND);
						System.out.println("PVSTBTZP5 = " + PVSTBTZP5 + " PVSTBTZP7 = "
								+ PVSTBTZP7 + " PVSVORSTART = " + PVSVORSTART
								+ " PVSVOREND = " + PVSVOREND + " PVSABSSTART = "
								+ PVSABSSTART + " PVSABSEND = " + PVSABSEND);
						System.out.println("OSTBTZP5 = " + OSTBTZP5 + " OSTBTZP7 = "
								+ OSTBTZP7 + " OSVORSTART = " + OSVORSTART
								+ " OSVOREND = " + OSVOREND + " OSABSSTART = "
								+ OSABSSTART + " OSABSEND = " + OSABSEND);
						System.out.println("sopIndex = " + sopIndex + " TPPA_ME = "
								+ TPPA_ME + " QFTPPA_Kunde = " + QFTPPA_Kunde);
					}
				%>
