...
/**
 * @author emily Implementation of the ExceptionHandler. For
 *         ConnectionExceptions (server temporarily down .etc) prompts the user
 *         to retry the last request. For other exceptions convert to a RunTime
 *         exception.
 */
public class AppXExceptionHandler implements ExceptionHandler {
	...
	public void handleException(InternalServerException ise) {
		...
		/*
		 * try { String retry = reader.readLine(); // If yes, return to the
		 * calling SOA client framework, where the // last service request will
		 * be resent. if (retry.equalsIgnoreCase("y") ||
		 * retry.equalsIgnoreCase("yes")) return;
		 * 
		 * throw new RuntimeException(
		 * "The user has opted not to retry the last request"); } catch
	...
	public void handleException(CanceledOperationException coe) {
		...
		System.out.println("*****");
		System.out
				.println("Exception caught in com.teamcenter.clientx.AppXExceptionHandler.handleException(CanceledOperationException).");
