...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	...
	<script type="text/javascript">
				...
				toolbar : [ {
					...
					handler : function() {
						...
						$.messager.confirm('确认删除项', '确认删除该选项', function(result){
							if (result){
								window.location.href='<%=request.getContextPath()%>/app/passing/pass/deletePassing.action?ids='+ids.join('__');
							}
						});
						return false;
					}
		...
		function deleteVO(id){
			$.messager.confirm('确认删除项', '确认删除该选项', function(result){
				if (result){
					window.location.href='<%=request.getContextPath()%>/app/passing/pass/deletePassing.action?ids=' + id;
				}
			});
			return false;
		}
	...
	<div id="panel" class="easyui-panel" title="查询条件"
		...
		<form id="queryForm" method="post">
			<table>
				<tr align="right">
					<td>站点编号:</td>
					<td align="left"><input id="station" name="station" type="text"></input></td>
					<td>卡编号:</td>
					<td align="left"><input id="card" name="card" type="text"></input></td>
					<td>创建时间:</td>
