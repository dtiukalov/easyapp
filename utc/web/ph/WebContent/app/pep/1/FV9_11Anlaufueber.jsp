		...
		<div id="container">
			...
			<%
				if (derviantList != null && derviantList.size() > 0) {
					...
					int defaultLength = 8; //默认8行
					
					double total_height = 400.0; //表格的总高度（不包括Title）
					
					double height = total_height/defaultLength; //每行默认高度
					
					double adjustHeight = 0.0; //用于调整高度
					
					//超过8行
					if (derviantList.size() > defaultLength) {
						//每行所占高度减少
						height = total_height/derviantList.size();
						//不需要调整高度
					}
					...
					if (derviantList.size() < defaultLength){
						//每行所占高度不变
						//通过调整高度使表格垂直方向居中
						adjustHeight = ((defaultLength - derviantList.size()) * height)/2.0;
					}
				System.out.println("height = " + height);
				System.out.println("adjustHeight = " + adjustHeight);
			%>
				<div style="width: 1000px; height:<%=adjustHeight%>px; margin: 0px; float: left;">&nbsp;</div>
				<table style="border: 2px solid black; width: 1000px;  margin: 20px 0px auto; 
					...
					border-collapse:collapse; border:1px solid; float: left;">
					<tr>
					...
					for (int l=0; l<derviantList.size(); l++) {
						...
						double floatnum = 5.0; //浮动数字
						double top = 220.0 + (height+1)*l + adjustHeight + floatnum; //通过adjustHeight调整绝对位置
